...
> - **Scope (per client, 2026-08-08): the official branding applies to the KEY GRAPHIC only for now.** The landing page keeps the movie-brand layout below; the official poster lives in the key-art slot as a single composed image (`images/poster-official.jpg`, built from the official assets). A full CSS-grid build of the poster exists in git history (commit `90f4121`) if the team ever wants the whole page in the official system.
